...
class LineSearchOptimizer(object):
    ...
    def _accelerate(self):
        if self._par.get('accelerator') is not None:
            x_extr = self._par['accelerator'](self.convergence, self._f)
            if self._par.get('choose_best') != True or self._f(x_extr) < self._f(self._x_current):
                self.convergence[-1] = self._x_current = x_extr
